...
re-patching never clicks or crashes. See [DAG §2.5](dag.md#live-editing-and-the-airlock).
...
currency of knobs, patches and MIDI. See [Parameters §6.1](parameters.md#cv).

### DAG (Directed Acyclic Graph)
The node-and-wire structure of a patch. Acyclic so a stable execution order exists. See
[The Node Graph](dag.md).
...
patch load. See [DAG §2.3](dag.md#compilation-from-your-wiring-to-the-execution-plan).
...
[Concepts §1.2](concepts.md#q1616--the-engines-internal-precision).
...
of the [Airlock](#airlock). See [Concepts §1.4](concepts.md#the-three-thread-model-and-why-it-shapes-the-feel).

### Staging pool
The Host-side mirror of a Part's nodes where parameter edits land before being atomically consumed
by the audio thread. The base/macro half of the [bifurcated model](parameters.md#bifurcation).
...
[Performance §3.8](performance.md#tempo).
...
`gdvp_voice_manager.c/.h`. See [Performance](performance.md).

### Voice
A single sounding instance of a patch, with its own node copies and state. 128 in the global pool;
up to 16 per Part. See [Concepts §1.1](concepts.md).
