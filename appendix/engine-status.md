...
(master domain, post-mix, monophonic) and produce audio. See [Effects](../sound/effects.md) and
...
similar effects-suffixed patches now render their GFX tails. See [Patches §7.4](../sound/patches.md#library).
...
- The GFX effect family (`GDVP_NODE_GFX_*`) is documented as active in `gdvp_nodes.h` and fully
  wired in the dispatch tables. See [Effects](../sound/effects.md) and [GFX nodes](../nodes/gfx.md).
...
The [arpeggiator](../concepts/performance.md#the-arpeggiator) *does* consume the transport clock, so tempo sync
works there. See [Performance §3.8](../concepts/performance.md#tempo).

---

## A.4 MIDI messages tracked but not acted on ⚠️

Received and stored in the MIDI matrix, but currently no engine action ([MIDI §8.5](../control/midi.md#stubs)):
...
- **DELAY voice mode** is a real, working strum/echo built from staggered voices — independent of
  the GFX delay line. See [Performance §3.6](../concepts/performance.md#strum--delay-allocation).
...
([Hosts §standalone](../control/hosts.md#standalone)). Not a bug — the licence gate.
