...
- Key Range: Visually displays multiple octaves (e.g., C4 through C9).
...
Parameters :
  ...
  - **Wet** : Audio that has been fully processed and altered by the effect. In the case of reverb, the wet signal is only the echoes and reflections generated by the node, completely stripping away the original direct sound. *Values : 0 to 100 %* 
  - **Dry** : Original, raw, un-processed audio. It is the exact sound generated by oscillators and filters before it hits any spatial effects like reverb, delay, or chorus. *Values : 0 to 100 %*

#### ROUTE
The ROUTE panel is a dedicated sub-tab (adjacent to PARAMS) that appears when a specific node is selected. It allows the user to change the source or destination of the signal at the selected node. 
